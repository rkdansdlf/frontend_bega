...
export default function Home({ onNavigate }: HomeProps) {
    ...
    return (
        ...
        <div className="min-h-screen bg-gray-50 dark:bg-gray-950 transition-colors duration-300">
            ...
            <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 -mt-16 relative z-20 space-y-8 pb-20">
                ...
                <Card className="border-0 shadow-xl bg-white/95 dark:bg-gray-900/95 backdrop-blur-md sticky top-4 z-30 ring-1 ring-black/5 dark:ring-white/10">
                    <div className="p-2 sm:p-4 flex flex-col md:flex-row items-center justify-between gap-4">
                        {/* Date Controls */}
                        <div className="flex items-center gap-2 bg-gray-100 dark:bg-gray-800 p-1 rounded-xl w-full md:w-auto justify-between md:justify-start">
                            <Button 
                                variant="ghost" 
                                size="icon" 
                                onClick={() => changeDate(-1)} 
                                className="hover:bg-white dark:hover:bg-gray-700 rounded-lg h-9 w-9"
                            >
                                <ChevronLeft className="w-5 h-5 text-gray-600 dark:text-gray-300" />
                            </Button>
                            
                            <div className="flex items-center gap-2 px-4 py-1">
                                <CalendarIcon className="w-4 h-4 text-[#2d5f4f] dark:text-[#4ade80]" />
                        ...
                        <div className="flex items-center gap-2 w-full md:w-auto">
                            <Button
                                ...
                                className="flex-1 md:flex-none border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-800"
                            >
                ...
                <Tabs value={activeLeagueTab} onValueChange={handleTabChange} className="w-full">
                    <div className="flex justify-center mb-6">
                        <TabsList className="bg-gray-200/50 dark:bg-gray-800/50 p-1 rounded-full">
                            <TabsTrigger value="regular" className="rounded-full px-6 data-[state=active]:bg-[#2d5f4f] data-[state=active]:text-white">정규시즌</TabsTrigger>
                            <TabsTrigger value="postseason" className="rounded-full px-6 data-[state=active]:bg-[#2d5f4f] data-[state=active]:text-white">포스트시즌</TabsTrigger>
                            <TabsTrigger value="koreanseries" className="rounded-full px-6 data-[state=active]:bg-[#2d5f4f] data-[state=active]:text-white">한국시리즈</TabsTrigger>
                        </TabsList>
                    </div>
                                        ...
                                        {currentGames.length === 0 ? (
                                            <Card className="border-dashed border-2 bg-transparent shadow-none py-16">
                                                <div className="flex flex-col items-center text-center text-muted-foreground">
                                                    ...
                                                    <h3 className="text-lg font-semibold mb-1">예정된 경기가 없습니다</h3>
                                                    <p className="text-sm">다른 날짜를 선택하거나 캘린더를 확인해보세요.</p>
                                                </div>
                                            </Card>
                                        ) : (
                                            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                ...
                <section>
                    <div className="flex items-center justify-between mb-6">
                        <div className="flex items-center gap-2">
                            <div className="bg-[#2d5f4f] p-2 rounded-lg">
                                ...
                                <table className="w-full text-sm text-left">
                                    <thead className="text-xs text-gray-500 uppercase bg-gray-50/50 dark:bg-gray-900/50 border-b dark:border-gray-700">
                                        <tr>
                                            ...
                                            <th className="px-6 py-4 font-medium">팀</th>
                                            <th className="px-6 py-4 font-medium text-right">경기</th>
                                            <th className="px-6 py-4 font-medium text-right">승/패/무</th>
                                            <th className="px-6 py-4 font-medium text-right">승률</th>
                                        </tr>
                                    </thead>
                                    <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
                                        {rankings.map((team) => (
                                            ...
                                            >
                                                ...
                                                <td className="px-6 py-4">
                                                    <div className="flex items-center gap-3">
                                                        <div className="flex-shrink-0 w-8 h-8 flex items-center justify-center">
                                                            {/* Team Logo component used here */}
                                                            <TeamLogo team={team.shortName || team.teamName} size={32} />
                                                        </div>
                                                ...
                                                <td className="px-6 py-4 text-right text-gray-600 dark:text-gray-400">
                                                    <span className="text-emerald-600 dark:text-emerald-400 font-medium">{team.wins}승</span>
                                                    <span className="mx-1.5 text-gray-300">/</span>
                                                    <span className="text-rose-600 dark:text-rose-400 font-medium">{team.losses}패</span>
                                                    <span className="mx-1.5 text-gray-300">/</span>
                                                    {team.draws}무
                                                </td>
