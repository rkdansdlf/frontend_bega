          ...
          <div className="px-6 py-6 space-y-2">
            {navItems.map((item) => (
              <button
                key={item.id}
                onClick={() => navigate(`/${item.id}`)}
                className={`block w-full text-left py-4 px-4 text-lg font-semibold rounded-md transition-colors ${theme === 'dark' ? 'text-gray-200' : 'text-gray-700'
                  }`}
                style={
                  location.pathname === `/${item.id}`
            ...
            <div className="border-t border-gray-200 dark:border-gray-700 my-6 pt-6 space-y-3">
              {isLoggedIn ? (
                <>
                  <button
                    onClick={() => navigate('/mypage')}
                    className={`user-profile-button2 items-center justify-center py-3 w-full rounded-full transition-all duration-200 group font-bold text-base ${theme === 'dark' ? 'text-white border-gray-700' : 'text-gray-900 border-gray-200'
                      }`}
                  >
